...
static void sdio_read_task(void const* pvParameters)
{
	...
	for (;;) {
		...
		/* Catalyst Tab: at most 50 ms, then the slave's interrupt register is read anyway. The in-band SDIO
		 * interrupt (on DAT1) was now and then missed on the Tab5: with an unbounded wait the host never read
		 * again (every packet and RPC reply stuck behind it), or stayed throttled after a missed
		 * STOP_THROTTLE, while Wi-Fi still said "connected". The bits stay latched in INT_RAW until cleared,
		 * so a poll finds what the interrupt didn't deliver. */
		res = g_h.funcs->_h_sdio_wait_slave_intr(sdio_handle, pdMS_TO_TICKS(50));
		...
		if (BIT(SDIO_INT_START_THROTTLE) & interrupts) {
			if (!wifi_tx_throttling) ESP_LOGW(TAG, "slave: throttle Wi-Fi tx");
			wifi_tx_throttling = 1;
		}

		if (BIT(SDIO_INT_STOP_THROTTLE) & interrupts) {
			if (wifi_tx_throttling) ESP_LOGW(TAG, "slave: throttle off");

...
void assist_feed(const cat_robot_t *r) { (void)r; }
...
void snap_init(void) {}
int snap_take(const cat_robot_t *r, const char *reason) { (void)r; (void)reason; return -1; }
bool snap_revert(const cat_robot_t *r, int id) { (void)r; (void)id; return false; }
int snap_list(snap_info_t *out, int max) { (void)out; (void)max; return 0; }
int snap_diff(const cat_robot_t *r, int id, char *out, int max) { (void)r; (void)id; if (max) out[0] = 0; return 0; }
